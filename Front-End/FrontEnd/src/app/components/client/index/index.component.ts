  ...
  register(): void {
    ...
    this.authService.register(username, email, password).subscribe({
      next: res => {
        ...
        // this.showSuccess(res['message']);  // Hiển thị thông báo từ server
        if (res['checkEmail']) {
          // Nếu cần kiểm tra email, có thể thực hiện một số hành động tương ứng ở đây
          this.showWarn("Xác thực email để hoàn thành đăng ký!");
        }
        this.authModal = false;
      },
      error: err => {
        this.showError("Kiểm tra lại thông tin đăng kí!!");
